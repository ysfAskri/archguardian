...
async function tryInstallHusky(gitRoot: string): Promise<InstallResult | null> {
  ...
  const archguardianLine = 'npx archguardian check';

  try {
    const existing = await readFile(hookPath, 'utf-8');
    if (existing.includes('archguardian')) {
      logger.info('archguardian already in husky pre-commit hook');
...
async function installDirectHook(gitRoot: string): Promise<InstallResult> {
  ...
  try {
    const existing = await readFile(hookPath, 'utf-8');
    if (existing.includes('archguardian')) {
      logger.info('archguardian already in pre-commit hook');
...
export async function isHookInstalled(cwd: string): Promise<boolean> {
  try {
    ...
    try {
      ...
      if (huskyHook.includes('archguardian')) return true;
    } catch { /* not husky */ }

    // Check direct hook
    try {
      const directHook = await readFile(join(gitRoot, '.git', 'hooks', 'pre-commit'), 'utf-8');
      if (directHook.includes('archguardian')) return true;
    } catch { /* no hook */ }
